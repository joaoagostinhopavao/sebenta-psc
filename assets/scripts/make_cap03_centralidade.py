...
from sebenta_style import (DARK, MAGENTA, PINK, GREY, NOTE_COLOR, apply_style,
                            ...
                            NODE_RADIUS, NODE_RADIUS_HI)

apply_style()

def draw_graph(ax, edges, pos, highlight, node_font=NODE_LABEL_SIZE, hi_font=NODE_LABEL_SIZE_HI,
               r=NODE_RADIUS, hi_r=NODE_RADIUS_HI):
    for u, v in edges:
        hi = (u in highlight or v in highlight)
        color = PINK if hi else GREY
        lw = 4.4 if hi else 3.0
...
# ------------------------------------------------------------------
# Painel 1: GRAU — uma estrela: um nó central com seis ligações diretas
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# Painel 2: INTERMEDIAÇÃO — dois grupos ligados por uma única ponte
# ------------------------------------------------------------------
edges2 = [("A","B"), ("A","C"), ("B","C"),
          ("D","E"), ("D","F"), ("E","F"),
          ("A","X"), ("X","D")]
...
# ------------------------------------------------------------------
# Painel 3: PROXIMIDADE — uma cadeia; o nó do meio chega a todos mais depressa
# ------------------------------------------------------------------
chain = ["A", "B", "C", "D", "E", "F", "G"]
...
panels = [
    ("Centralidade de grau", edges1, pos1, highlight1,
     "H liga-se diretamente a todos os outros; os restantes têm apenas uma ligação"),
    ("Centralidade de intermediação", edges2, pos2, highlight2,
     "X está no único caminho entre os dois grupos, apesar de\nter poucas ligações diretas"),
    ("Centralidade de proximidade", edges3, pos3, highlight3,
     "D está, em média, mais perto de todos os outros nós do que qualquer outro ponto da cadeia"),
]

fig, axes = plt.subplots(3, 1, figsize=(11, 17.5), dpi=200)

xlims = [(0.3, 4.7), (-0.4, 5.0), (-0.3, 5.1)]
ylims = [(-0.9, 3.1), (-0.35, 2.15), (0.1, 2.1)]
...
for ax, (title, edges, pos, highlight, note), xlim, ylim in zip(axes, panels, xlims, ylims):
    ...
    ax.text(0.0, -0.06, note, ha="left", va="top", fontsize=NOTE_SIZE, color=NOTE_COLOR, style="italic",
            transform=ax.transAxes, wrap=True)

fig.tight_layout(h_pad=5.5)
fig.savefig("/Users/jpavao/QProjects/Sebenta-PSC/assets/images/cap03/centralidade.png",
