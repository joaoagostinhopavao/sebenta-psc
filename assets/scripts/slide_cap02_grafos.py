...
from sebenta_style import (DARK, MAGENTA, PINK, NOTE_COLOR, apply_style,
                            ...
                            SLIDE_NOTE_SIZE, SLIDE_REFERENCE_WIDTH)

apply_style()


def node(ax, xy, label, color=DARK, r=0.17, fontsize=SLIDE_NODE_LABEL_SIZE, textcolor="white"):
...
def edge(ax, p1, p2, color=MAGENTA, lw=2.6, alpha=1.0):
    ax.plot([p1[0], p2[0]], [p1[1], p2[1]], color=color, linewidth=lw,
             zorder=2, alpha=alpha, solid_capstyle="round")


# 2 painéis apenas -> cada um tem ~6.7in num slide 16:9, mais folga do
# que as grelhas de 3-4 painéis; usam-se as constantes SLIDE_ base.
fig, axes = plt.subplots(1, 2, figsize=(SLIDE_REFERENCE_WIDTH, 6.6), dpi=200)
...
for name, xy in people.items():
    node(ax, xy, name, color=DARK, r=0.19, fontsize=SLIDE_NODE_LABEL_SIZE)

ax.set_title("Grafo social", fontsize=SLIDE_TITLE_SIZE, fontweight="bold", color=DARK, pad=14)
ax.text(2.0, -0.55, "ligações explícitas: quem segue quem,\nquem conhece quem",
        ha="center", va="top", fontsize=SLIDE_NOTE_SIZE, color=NOTE_COLOR)
...
for topic, xy in interests.items():
    for uxy in users_by_interest[topic]:
        edge(ax, xy, uxy, color=PINK, lw=2.2, alpha=0.9)

for topic, xy in interests.items():
    node(ax, xy, "", color=DARK, r=0.22)
    if label_side[topic] == "acima":
        ax.text(xy[0], xy[1] + 0.4, topic, ha="center", va="bottom",
                fontsize=SLIDE_NODE_LABEL_SIZE_HI, color=DARK, fontweight="bold")
    else:
...
ax.set_title("Grafo de interesses", fontsize=SLIDE_TITLE_SIZE, fontweight="bold", color=DARK, pad=14)
ax.text(2.0, -0.55, "agrupamento por comportamento:\nquem consome o quê",
        ha="center", va="top", fontsize=SLIDE_NOTE_SIZE, color=NOTE_COLOR)

fig.tight_layout(rect=[0, 0.02, 1, 1])
fig.savefig("/Users/jpavao/QProjects/Sebenta-PSC/assets/images/cap02/slides/grafo-social-vs-interesses-slide.png",
            facecolor="white")
print("done")
